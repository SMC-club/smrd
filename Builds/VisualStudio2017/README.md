...
We do not recommend Windows for smrd production use at this time. Currently,
...
modification, build smrd under Visual Studio, and run the unit tests you will
...
Any version of Visual Studio 2017 may be used to build smrd. The **Visual
...
Building smrd requires **protoc.exe** version 2. Version 3 is not currently
...
* NOTE: Since smrd links statically to OpenSSL, it does not matter where the
  OpenSSL .DLL files are placed, or what version they are. smrd does not use
...
The smrd application is linked statically to the standard runtimes and
...
process and clone `git@github.com:CryptoManiac/smrd.git`. Otherwise follow this
...
above. Navigate to the directory where you want to clone smrd (git bash uses
...
5. Select either the `smrd.exe` (unity) or `smrd_classic.exe` (non-unity)
   ...
   smrd repo, e.g. `datagen.vcxproj`. Please ignore them.
...
```
.\build\x64-Release\Release\smrd(_classic).exe
```
or
```
.\build\x64-Debug\Debug\smrd(_classic).exe
```
...
execute the following commands within your `smrd` cloned repository:
...
Navigate to the `build\cmake` folder created above and select the `smrd.sln`
file. You can then choose whether to build the `Debug` or `Release` solution
configuration. Within the **Solution Explorer**, selected either the `smrd`
(unity build) project or the `smrd_classic` (non-unity) project, and
right-click to build.

The executable will be in 
```
.\build\cmake\Release\smrd(_classic).exe
```
 or
````
.\build\cmake\Debug\smrd(_classic).exe
````
where these paths are relative to your cloned git repository.
